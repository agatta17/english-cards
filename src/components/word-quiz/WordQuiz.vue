          ...
          <v-carousel-item v-for="(word, i) in wordsWithOptions" :key="i">
            <transition name="slide-up" mode="out-in">
              <full-card
                ...
                :is-show-full-card.sync="isShowFullCard"
              />

              <div v-else class="d-flex justify-center wrap">
                <v-sheet
                  class="px-5 pt-1 pb-5 relative"
                  :width="isMobile ? '100%' : '400px'"
                  :outlined="!isMobile"
                  rounded
                >
                  <v-btn @click="isShowFullCard = true" icon absolute right>
                    <v-icon color="sky">
                      {{
                        isShowFullCard
